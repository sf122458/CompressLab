"""
Image codec used to compress the I frame in video compression.
...
# mentioned in DCVC-TCM, remove the autoregressive part
class Cheng2020(CompressionModel):
    ...
    def compress(self, x):
        ...
        out = self.decompress([y_strings, z_strings], z.size()[-2:])

        return {"strings": [y_strings, z_strings], "shape": z.size()[-2:], "x_hat": out["x_hat"]}
...
def load_model(architecture, quality, **kwargs):
    return model_architectures[architecture](*cfgs[architecture][quality], **kwargs)