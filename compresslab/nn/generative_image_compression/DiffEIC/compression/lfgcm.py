"""
The implementation of the LFGCM model in DiffEIC, which is used for image compression.
"""
...
from .layers import SFT, SFTResidualBlock, ResidualBlockWithStride, ResidualBlockUpsample, ResidualBottleneck

    
class Encoder(nn.Module):
    """The encoder module in LFGCM.
    """
    def __init__(self, in_nc, mid_nc, out_nc, prior_nc, sft_ks):
        ...
        self.g_a4 = SFTResidualBlock(mid_nc[3], prior_nc, ks=sft_ks)
        self.g_a5 = SFTResidualBlock(mid_nc[3], prior_nc, ks=sft_ks)

        self.g_a6 = conv3x3(mid_nc[3], out_nc)

    def forward(self, x, feature):
        """The forward pass of the encoder module in LFGCM.

        Args:
            x (torch.Tensor): The input image.
            feature (torch.Tensor): Feature map extracted from 
                the pretrained stable diffusion's encoder.
        """
...
class Decoder(nn.Module):
    def __init__(self, N, M, out_nc, prior_nc, sft_ks):
        ...
        self.g_s0 = SFTResidualBlock(M, prior_nc, ks=sft_ks)
        self.g_s1 = SFTResidualBlock(M, prior_nc, ks=sft_ks)
...
class HyperEncoder(nn.Module):
    def __init__(self, N, M, prior_nc, sft_ks):
        ...
        self.h_a6 = conv(N, N)
        self.h_a7 = SFTResidualBlock(N, prior_nc, ks=sft_ks)
        self.h_a8 = SFTResidualBlock(N, prior_nc, ks=sft_ks)
...
class LFGCM(CompressionModel):
    """The latent feature-guided compression module in DiffEIC.
    """
